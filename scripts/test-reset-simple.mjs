/**
 * Simple test script for database reset functionality using ES modules
 * Run with: node scripts/test-reset-simple.mjs
 */
...
async function testResetFunctionality() {
  ...
  try {
    ...
    console.log('💡 Make sure node-fetch is installed: npm install node-fetch');
  }
}
