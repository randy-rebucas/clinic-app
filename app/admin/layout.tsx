...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  useEffect(() => {
    if (!loading && (!user || employee?.role !== 'admin')) {
      router.push('/');
    }
  ...
  const navigation = [
    ...
    { name: 'Lab Orders', href: '/admin/lab-orders', icon: FlaskConical },
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="fixed inset-y-0 left-0 flex flex-col w-64 bg-white shadow-xl">
          ...
          <div className="flex-shrink-0 flex items-center justify-between border-t border-gray-200 p-4">
            ...
            <button
              onClick={logout}
              className="ml-2 flex items-center text-gray-400 hover:text-gray-600 transition-colors"
            >
        ...
        <div className="flex flex-col w-64">
          <div className="flex flex-col h-full flex-1 border-r border-gray-200 bg-white">
            ...
            <div className="flex-shrink-0 flex items-center justify-between border-t border-gray-200 p-4">
              ...
              <button
                onClick={logout}
                className="ml-2 flex items-center text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0"
              >
                <LogOut className="h-5 w-5" />
              </button>
            </div>
      ...
      <div className="flex flex-col flex-1 lg:pl-64">
        {/* Top bar */}
        <div className="sticky top-0 z-10 lg:hidden flex-shrink-0 bg-white border-b border-gray-200 px-4 py-3">
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-md text-gray-500 hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 p-2"
